...
const createVehicle = async (req, res) => {
  try {
    ...
    // Auto-create VEHICLE type stock location for this vehicle
    try {
      const vehicleName = vehicle.vehicleName || "";
      const locationName = `${vehicle.registrationNumber} ${vehicleName}`.trim();
      ...
      if (location) {
        vehicle.stockLocationId = location._id;
        await vehicle.save();
      }
    } catch (locationError) {
      console.error("Warning: Could not create stock location for vehicle:", locationError);
      // Don't fail the vehicle creation if location fails
    }
...
const getVehicles = async (req, res) => {
  try {
    ...
    const vehicles = await Vehicle.find(filter).populate("stockLocationId").sort({ createdAt: -1 });
...
const updateVehicle = async (req, res) => {
  try {
    ...
    const vehicle = await Vehicle.findOneAndUpdate({ _id: req.params.id, agencyId: req.user.agencyId }, updates, {
      new: true,
      runValidators: true,
    });
...
const toggleVehicleStatus = async (req, res) => {
  try {
    ...
    if (vehicle.stockLocationId) {
      const StockLocation = require("../models/StockLocation");
      await StockLocation.findByIdAndUpdate(vehicle.stockLocationId, { isActive }, { new: true });
    }
